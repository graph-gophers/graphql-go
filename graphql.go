...
// ConditionalIntrospection accepts a filter func. If this function returns false the introspection is disabled, otherwise it is enabled.
// If this option is not provided the introspection is enabled by default.
// This is useful for allowing introspection only to admin users, for example:
//
...
//	}
//
// Do not use it together with [DisableIntrospection], otherwise the option added last takes precedense.
func ConditionalIntrospection(fn func(ctx context.Context) bool) SchemaOpt {
	return func(s *Schema) {
...
//	   return false
//	}
//	graphql.ConditionalIntrospection(filter)
//
// Deprecated: use [ConditionalIntrospection] filter instead. Do not use it together with [ConditionalIntrospection], otherwise the option added last takes precedense.
...
func (s *Schema) exec(ctx context.Context, queryString string, operationName string, variables map[string]interface{}, res *resolvable.Schema) *Response {
	...
	r := &exec.Request{
		Request: selected.Request{
			...
			Vars:                 variables,
			Schema:               s.schema,
			DisableIntrospection: s.allowIntrospection != nil && !s.allowIntrospection(ctx), // allow introspection by default, i.e. when allowIntrospection is nil
		},
