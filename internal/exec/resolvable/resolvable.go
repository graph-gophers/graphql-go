...
func ApplyResolver(s *types.Schema, resolver interface{}) (*Schema, error) {
	...
	for _, op := range [...]string{Query, Mutation, Subscription} {
		...
		if m.IsValid() {
			...
			if len(out) != 1 {
				return nil, fmt.Errorf("method %q of %v must have 1 return value, got %d", op, rv.Type(), len(out))
			}
			res := out[0]
			switch res.Kind() {
			...
			default:
				return nil, fmt.Errorf("method %q of %v must return an interface or a pointer, got %v", op, rv.Type(), res.Kind())
			}
		}
