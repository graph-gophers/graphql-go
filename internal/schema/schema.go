...
// Schema, Object, Input, and Scalars can be extended.
//
...
func mergeExtensions(s *Schema) error {
	for _, ext := range s.extensions {
		...
		if typ == nil {
			return fmt.Errorf("trying to extend of unknown type %q", ext.Type.TypeName())
		}

		if typ.Kind() != ext.Type.Kind() {
			return fmt.Errorf("trying to extend of type %q with type %q", typ.Kind(), ext.Type.Kind())
		}
		...
		case *Object:
			...
			for _, field := range e.Fields {
				if og.Fields.Get(field.Name) != nil {
					return fmt.Errorf("extended field already available %q", field.Name)
				}
			}
			...
			for _, en := range e.interfaceNames {
				for _, on := range og.interfaceNames {
					if on == en {
						return fmt.Errorf("interface implemented in extension is already implemented %q for %q", on, og.Name)
					}
				}
			}
		...
		case *Union:
			...
			for _, en := range e.typeNames {
				for _, on := range og.typeNames {
					if on == en {
						return fmt.Errorf("union type provided again in extension %q for %q", on, og.Name)
					}
				}
			}
